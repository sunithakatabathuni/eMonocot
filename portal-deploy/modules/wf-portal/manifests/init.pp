class wf-portal ($version) {
}

define wf-portal::wf-portal (
	$version = '1.0.6',
	...
	file {"/var/lib/tomcat7/common/classes/META-INF/spring/emonocot-portal.properties":
		ensure  => file,
		content => template('wf-portal/emonocot-portal.properties.erb'),
	} ->

	file {"/var/lib/tomcat7/common/classes/META-INF/spring/emonocot-portal-db.properties":
		ensure  => file,
		content => template('wf-portal/emonocot-portal-db.properties.erb'),
	} ->

	file {"/var/lib/tomcat7/common/classes/META-INF/spring/emonocot-portal-messages.properties":
		ensure  => file,
		content => template('wf-portal/emonocot-portal-messages.properties.erb'),
	} ->

	file {"/var/lib/tomcat7/common/classes/logback-emonocot-portal.xml":
		ensure  => file,
		content => template('wf-portal/logback-emonocot-portal.xml.erb'),
	}

	# WAR & static file binaries
	file { "/var/lib/tomcat7/webapps/ROOT.war":
		owner => tomcat7,
		group => tomcat7,
		ensure => file,
		source => "puppet:///modules/wf-portal/emonocot-portal-${version}.war",
	}

	file { "/var/tmp/world-flora-static.tar.gz":
		owner => tomcat7,
		group => tomcat7,
		ensure => file,
		source => "puppet:///modules/wf-portal/emonocot-static-${version}-world-flora.tar.gz",
	}
	...
	/**
	 * Install an Apache configuration file which forwards requests
	 * by default to Tomcat with the exception of static resources
	 */
	$static_dirs = ["images", "css", "js", "sitemap", "tiles", "output"]
	$geo_server = "localhost"
	$tomcat_server =  "localhost:8009"

	include common::apache

	file { "/etc/apache2/conf-enabled/tomcat.conf" :
		ensure  => file,
		content => template("wf-portal/tomcat.conf.erb"),
	...
	file { "/etc/apache2/mods-enabled/proxy_ajp.load":
		ensure => link,
		target => "../mods-available/proxy_ajp.load",
		notify => Service["apache2"],
	}
	...
	exec { "unpack-static":
		path => "/bin:/usr/bin",
		command => "tar -xzf /var/tmp/world-flora-static.tar.gz && touch .extract-time",
		cwd => "/var/www/",
		onlyif => "test /var/tmp/world-flora-static.tar.gz -nt .extract-time",
	}
